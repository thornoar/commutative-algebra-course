...
#pf[\
  ...
  $
    phi(a_1, a_2, ..., a_n) = a_1 x_1 + a_2 x_2 + ... + a_n x_n.
  $
...
#lm[
  Пусть $M$ --- конечно порождённый $A$-модуль, $aa$ --- некоторый идеал в $A$, а $phi : M -> M$ --- такой эндоморфизм, что $phi(M) subset aa M$. Тогда найдутся такие элементы $alpha_1, ..., alpha_n in aa$, что
  $
    phi^n + beta_1 phi^(n-1) + ... + beta_(n-1) phi + beta_n = 0 in "End"(n).
  $
] <equation>
#pf[
...
#cor("лемма Накаямы")[
  Пусть $M$ --- конечно порождённый $A$-модуль, $aa <= A$ --- идеал, содержащийся в радикале Джекобсона $frak(R)$ кольца $A$. Тогда если $alpha M = M$, то $M = 0$.
] #pf("1")[
  В силу предыдущего следствия, $alpha M = 0$ для некоторого $alpha equiv 1 #h(1mm) (mod frak(R))$. По @frakr-description[лемме], из этого следует, что $alpha$ обратим в $A$. Поэтому $M = alpha^(-1) alpha M = 0$.
] #pf("2")[
  ...
  $
  Поскольку $alpha_n in frak(R)$, из @frakr-description[леммы] следует, что $1 - alpha_n$ обратимо в $A$. Следовательно, $u_n$ выражается через остальные $u_1, ..., u_(n-1)$, что противоречит минимальности системы ${u_i}$.
]

#cor[
  Пусть $M$ --- конечно порождённый $A$-модуль, $N$ --- некоторый подмодуль $M$, и $aa subset frak(R)$ --- идеал. Тогда
  $
    M = aa M + N ==> M = N.
  $
] #pf[
...
#rem[
  ...
  $
  При надобности, нетрудно проверить, что это естественное соответствие линейно.
]
