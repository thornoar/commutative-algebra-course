...
#def("подкольца и идеалы")[
  ...
  + Для любого $x in A$, множество $x A = {x y | y in A}$ образует идеал, который обозначается $(x)$. Идеалы вида $(x)$ называются _главными._
]
...
#stat[
  Существует биекция
  $
    tilde(phi) : { bb <= A | aa subset bb } <-> {overline(bb) <= A\/aa},
  $
...
#cor[
  Каждый собственный идеал $aa subset A$ содержится в некотором максимальном идеале, и всякий необратимый элемент содержится в некотором максимальном идеале.
] <all-contained>
#pf[
...
#def[
  ...
  + _Произведением_ двух идеалов $aa$ и $bb$ называется идеал, порождённый всевозможными произведениями $x y$, где $x in aa$, $y in bb$:
    $
      aa dot bb = gen({x y | x in aa, y in bb}) = {sum_(i = 1)^n x_i y_i | x_i in aa, y_i in bb}.
    $
]
...
#exer[
  Правда ли, что всякий простой идеал $pp$ взаимно прост с любым другим идеалом $aa != (0)$?
]
