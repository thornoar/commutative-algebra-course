...
#stat[
  ...
  Очевидно, что если $x in cal(N)$, то $a x in cal(N)$ для любого $a in A$. Теперь рассмотрим два элемента $x, y in cal(N)$, причём $x^n = 0$ и $y^m = 0$. Тогда выражение $(x + y)^(m+n)$ по теореме Ньютона раскрывается следующим образом:
  $
...
#pf[
  ...
  $
    r(a) = phi^(-1)(cal(N)_(A\/aa)) = phi^(-1)(inter.big#h(1mm) {pp <= A\/aa mid(|) pp "прост в" A\/aa}) = inter.big#h(1mm) {phi^(-1)(pp) mid(|) pp "прост"} =\
...
#cor("лемма Накаямы")[
  Пусть $M$ --- конечно порождённый $A$-модуль, $aa <= A$ --- идеал, содержащийся в радикале Джекобсона $cal(R)$ кольца $A$. Тогда если $alpha M = M$, то $M = 0$.
] #pf("1")[
